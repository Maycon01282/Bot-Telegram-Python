...
def list_order_items_view(request, order_id=None):
    """
    Lista todos os itens de um pedido específico com paginação.
    """
...
@swagger_auto_schema(
    ...
    responses={201: OrderItemSerializer(), 400: 'Bad Request'}
)
@api_view(['POST'])
@permission_classes([IsAuthenticated])
...
@swagger_auto_schema(
    method='put',
    request_body=OrderItemSerializer,
    responses={200: OrderItemSerializer(), 400: 'Bad Request'}
)
@swagger_auto_schema(
    method='patch',
    request_body=OrderItemSerializer,
